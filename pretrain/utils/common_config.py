...
def get_train_dataset(p, transform=None):
    if p['train_db_name'] == 'VOCSegmentation':
        ...
        return VOCSegmentation(root=Path.db_root_dir(p['train_db_name']),
                            ...
                            transform=transform)
    if p['train_db_name'] == 'CATSNDOGS':
        print("CATSNDOGS")
        from data.dataloaders.catsndogs import CATSNDOGS_Segmentation
        return CATSNDOGS_Segmentation(root=Path.db_root_dir(p['train_db_name']),
                            saliency=p['train_db_kwargs']['saliency'],
                            transform=transform)
    if p['train_db_name'] == 'CATSNDOGS_PICANET':
        print("CATSNDOGS_PICANET")
        from data.dataloaders.catsndogs_picanet import CATSNDOGS_PICANET_Segmentation
        return CATSNDOGS_PICANET_Segmentation(root=Path.db_root_dir(p['train_db_name']),
                            saliency=p['train_db_kwargs']['saliency'],
                            transform=transform)
    if p['train_db_name'] == 'IMAGENET_SAMPLE':
        print("IMAGENET_SAMPLE")
        from data.dataloaders.imagenet_sample import IMAGENET_SAMPLE_Segmentation
        return IMAGENET_SAMPLE_Segmentation(root=Path.db_root_dir(p['train_db_name']),
                            saliency=p['train_db_kwargs']['saliency'],
                            transform=transform)
    if p['train_db_name'] == 'IMAGENET_SAMPLE_PICANET':
        print("IMAGENET_SAMPLE_PICANET")
        from data.dataloaders.imagenet_sample_picanet import IMAGENET_SAMPLE_PICANET_Segmentation
        return IMAGENET_SAMPLE_PICANET_Segmentation(root=Path.db_root_dir(p['train_db_name']),
                            saliency=p['train_db_kwargs']['saliency'],
                            transform=transform)
...
def get_train_transformations(p):
    augmentation = [
        ...
        transforms.ToTensor(),
        transforms.Normalize(mean=p['mean'],
                                 std=p['std'])

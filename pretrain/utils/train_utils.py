...
def train(p, train_loader, model, optimizer, epoch, amp):
    ...
    for i, batch in enumerate(train_loader):
        # Forward pass
        im_q = batch['query']['image'].to(p['gpu'], non_blocking=True)
        im_k = batch['key']['image'].to(p['gpu'], non_blocking=True)
        sal_q = batch['query']['sal'].to(p['gpu'], non_blocking=True)
        sal_k = batch['key']['sal'].to(p['gpu'], non_blocking=True)

        logits, labels, saliency_loss = model(im_q=im_q, im_k=im_k, sal_q=sal_q, sal_k=sal_k)

        # Use E-Net weighting for calculating the pixel-wise loss.
        uniq, freq = torch.unique(labels, return_counts=True)
        p_class = torch.zeros(logits.shape[1], dtype=torch.float32).to(p['gpu'], non_blocking=True)

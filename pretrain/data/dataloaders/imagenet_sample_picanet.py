...
class IMAGENET_SAMPLE_PICANET_Segmentation(torch.utils.data.Dataset):

    def __init__(self, root=Path.db_root_dir('IMAGENET_SAMPLE_PICANET'),
                 saliency='supervised_model',
                 transform=None, overfit=False):
        super(IMAGENET_SAMPLE_PICANET_Segmentation, self).__init__()
        ...
        for f in all_:
            ...
            _sal = os.path.join(self.sal_dir, f + ".jpg")
    ...
    def __str__(self):
        return 'IMAGENET_SAMPLE_PICANET(saliency=' + self.saliency + ')'
...
if __name__ == '__main__':
    ...
    dataset = IMAGENET_SAMPLE_PICANET_Segmentation(saliency='supervised_model')

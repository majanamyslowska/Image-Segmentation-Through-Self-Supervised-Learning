...
def get_train_dataset(p, transform=None):
    ...
    elif p['train_db_name'] == 'oxford-iiit-pet':
        from data.dataloaders.oxoford  import Oxford
        dataset = Oxford(split=p['train_db_kwargs']['split'], transform=transform)

    elif p['train_db_name'] == 'oxford-iiit-pet-70':
        from data.dataloaders.oxoford  import Oxford
        dataset = Oxford(split=p['train_db_kwargs']['split'], transform=transform, root=Path.db_root_dir('oxford-iiit-pet-70'))
    elif p['train_db_name'] == 'oxford-iiit-pet-50':
        from data.dataloaders.oxoford  import Oxford
        dataset = Oxford(split=p['train_db_kwargs']['split'], transform=transform, root=Path.db_root_dir('oxford-iiit-pet-50'))
...
def get_val_dataset(p, transform=None):
    ...
    elif p['val_db_name'] == 'oxford-iiit-pet':
        from data.dataloaders.oxoford import Oxford
        dataset = Oxford(split='val', transform=transform)     
    
    elif p['train_db_name'] == 'oxford-iiit-pet-70':
        from data.dataloaders.oxoford  import Oxford
        dataset = Oxford(split='val', transform=transform, root=Path.db_root_dir('oxford-iiit-pet-70'))
    elif p['train_db_name'] == 'oxford-iiit-pet-50':
        from data.dataloaders.oxoford  import Oxford
        dataset = Oxford(split='val', transform=transform, root=Path.db_root_dir('oxford-iiit-pet-50'))
...
def get_train_transformations(p,augmentation_strategy='pascal'):
    return transforms.Compose([custom_tr.RandomHorizontalFlip(),
                                   ...
                                   custom_tr.FixedResize(resolutions={'image': tuple((512,512)), 'semseg': tuple((512,512))},
                                    flagvals={'semseg': cv2.INTER_NEAREST, 'image': cv2.INTER_CUBIC}),
                                   custom_tr.ToTensor(),
                                    custom_tr.Normalize(p["mean"],p["std"])])

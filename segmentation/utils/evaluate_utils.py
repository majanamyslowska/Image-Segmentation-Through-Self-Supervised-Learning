...
def eval_segmentation_supervised_online(p, val_loader, model, verbose=True):
    ...
    for i, batch in enumerate(val_loader):
        ...
        targets = batch['semseg'].to(device, non_blocking=True)
        output = model(images)
...
def save_results_to_disk(p, val_loader, model, crf_postprocess=False):
    ...
    for i, batch in enumerate(val_loader):
        ...
        for jj in range(output.shape[0]):
            ...
            # Save using PIL
            # print(f"PATH: {os.path.join(p['save_dir'], meta['image'][jj] + '.png')}")#stop big print here
            pil_image.save(os.path.join(p['save_dir'], meta['image'][jj] + '.png')) 
